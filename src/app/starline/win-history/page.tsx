...
export default function StarlineWinHistoryPage() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      const starlineWins = allUserBids
        .filter((bid: any) => bid.gameName === 'KALYAN 777 STARLINE' && bid.status === 'won')
        .sort((a: any, b: any) => {
      ...
      setWins(starlineWins);
      setLoading(false);
    }, (error) => {
      console.error("Error fetching Starline wins:", error);
      setLoading(false);
    });
    ...
    <div className="flex flex-col min-h-screen bg-gray-50 pb-20">
      <header className="sticky top-0 z-50 w-full p-4 flex items-center gap-4 text-white bg-header">
        ...
        <h1 className="text-lg font-bold flex items-center gap-2">
          ...
          KALYAN 777 Starline Win Report
        </h1>
      </header>
      ...
      <main className="flex-1 p-4 space-y-4">
        {loading ? (
          ...
          <div className="text-center py-20 text-gray-500">
            <p>No winning records found for KALYAN 777 Starline.</p>
          </div>
        ) : (
          wins.map((win) => (
              ...
              <CardContent className="p-4">
                <div className="grid grid-cols-2 gap-4">
                  <div className="flex flex-col">
                    <span className="text-[10px] text-gray-400 uppercase font-bold">Game Slot</span>
                ...
                <div className="mt-3 pt-3 border-t flex justify-between items-center">
                  <span className="text-xs text-gray-500">Number: <span className="font-bold text-gray-800">{win.numbers.join(', ')}</span></span>

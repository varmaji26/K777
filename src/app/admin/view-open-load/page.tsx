...
import { Play, Calendar as CalendarIcon, Gamepad2, Copy, Search } from 'lucide-react';
...
export default function ViewOpenLoadPage() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(bidsQuery, (bidsSnapshot) => {
      ...
      const openBids = bidsData.filter(bid => bid.session === 'Open' && bid.status !== 'cancelled');
  ...
  const filteredBids = useMemo(() => {
    if (!userSearch.trim()) return allOpenBids;
    const term = userSearch.toLowerCase().trim();
    return allOpenBids.filter(bid => 
        bid.displayName?.toLowerCase().includes(term) || 
        bid.mobile?.includes(term)
    );
  }, [allOpenBids, userSearch]);
      ...
      <div className="grid gap-6">
        <Card>
          <CardHeader>
              ...
              <CardDescription>View live bidding load. Use user search to see individual player bets.</CardDescription>
        ...
        {selectedGame && (
            <div className="space-y-6">
                <h2 className="text-xl font-bold text-blue-900 border-b-2 border-blue-100 pb-2 flex items-center gap-2">
                    <Play className="fill-blue-900 h-5 w-5" />
                    DETAILS FOR: {selectedGame.name.toUpperCase()} (OPEN)

...
import { betTypes, BetType } from '@/lib/types';
...
const CustomWalletIcon = ({ className }: { className?: string }) => (
  <svg viewBox="0 0 512 512" className={className} xmlns="http://www.w3.org/2000/svg">
    ...
    <path style={{ fill: '#F6B545' }} d="M286.08,30.076c-18.752,18.736-18.776,49.128-0.04,67.88c0.016,0.016,0.024,0.024,0.04,0.04 l67.92-67.92c-18.736-18.752-49.128-18.776-67.88-0.04C286.104,30.052,286.096,30.06,286.08,30.076z" />
  </svg>
);
...
export default function PlayLayout({
  ...
  const betTypeKey = typeof params.betType === 'string' && Object.keys(betTypes).includes(params.betType) ? params.betType as BetType : null;
  const betTypeLabel = betTypeKey ? betTypes[betTypeKey] : game?.name;
    ...
    <div className="flex flex-col min-h-screen">
      <header className="sticky top-0 z-40 w-full border-b bg-header text-white">
        <div className="container flex h-16 items-center justify-between gap-4">
          <h1 className="font-bold text-lg whitespace-nowrap overflow-hidden text-ellipsis mx-auto">
            {(betTypeLabel || game.name).toUpperCase()}
          </h1>
          <Button variant="outline" className="bg-background/10 border-white/20 text-white hover:bg-white/20 h-8 px-3 pointer-events-none shrink-0 gap-1">
            ...
            <span className="font-bold text-white">{currentUser.balance?.toFixed(0) ?? '0'}</span>
          </Button>
        </div>
      </header>

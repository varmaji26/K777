...
export function TriplePanaForm({ game, betType }: BettingFormProps) {
  ...
  const minBid = appSettings.minBidTriplePana || 10;
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: {
      ...
      classicBids: allTriplePanas.map(pana => ({ pana, points: undefined })),
      advancedBidNumber: '',
  ...
  const handleAddAdvancedBid = () => {
    ...
    if (!allTriplePanas.includes(number)) {
        toast({ title: 'Invalid Pana', description: 'Invalid Triple Pana.', variant: 'destructive' });
        return;
    }

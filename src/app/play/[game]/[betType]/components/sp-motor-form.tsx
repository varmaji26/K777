...
const formSchema = z.object({
    ...
    number: z.string()
      .min(3, "Please enter at least 3 unique digits.")
...
export function SpMotorForm({ game, betType }: BettingFormProps) {
  ...
  const minBid = appSettings.minBidSpMotor || 10;
  ...
  const handleGenerate = (data: FormValues) => {
    ...
    const combinations = getCombinations(digits, 3);
    const generatedPanas = combinations.map(c => c.join(''));
    ...
    const newBids: BidItem[] = generatedPanas.map(p => ({ number: p, amount: numPoints }));

    setSubmittedBids(prev => [...prev, ...newBids]);
    toast({ title: "Bids Generated", description: `${newBids.length} SP Motor bids added.` });
    form.reset({
    ...
    try {
        await runTransaction(db, async (transaction) => {
            ...
            await logTransaction({
                ...
                type: 'bid_placed',
                description: `Bid on ${game.name} (SP Motor, ${session}). Total Panas: ${bidsToSubmit.length}`,

...
export default function JackpotBidHistoryPage() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      // Filter for Jackpot and sort on client side
      const jackpotBids = allUserBids
        .filter((bid: any) => bid.isJackpot === true || bid.gameName === 'KALYAN 777 JACKPOT')
        .sort((a: any, b: any) => {
    ...
    <div className="flex flex-col min-h-screen bg-gray-50 pb-20">
      <header className="sticky top-0 z-50 w-full p-4 flex items-center gap-4 text-white bg-header">
        ...
        <h1 className="text-lg font-bold flex items-center gap-2">
          ...
          KALYAN 777 Jackpot Bid History
        </h1>
      </header>
      ...
      <main className="flex-1 p-4 space-y-4">
        {loading ? (
          ...
          <div className="text-center py-20 text-gray-500">
            <p>No bids found for KALYAN 777 Jackpot games.</p>
          </div>
        ) : (
          bids.map((bid) => (
            <Card key={bid.id} className="overflow-hidden shadow-sm border-none">
              <CardHeader className="bg-muted/30 p-3 flex flex-row justify-between items-center">
                <span className="text-xs font-bold text-blue-900">KALYAN 777 JACKPOT - {bid.session}</span>
              ...
              <CardContent className="p-3">
                <div className="grid grid-cols-3 gap-2 text-center">
                  <div>
                    <p className="text-[9px] text-muted-foreground uppercase font-bold">Jodi</p>

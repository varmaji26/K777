...
export default function JackpotGamesPage() {
  ...
  useEffect(() => {
    if (jackpotGames.length > 0 || true) setLoading(false);
  }, [jackpotGames]);
  ...
  const isGameRunning = (timeStr: string) => {
    ...
    return currentTotalMinutes < (gameMinutes - 5); // Jackpot closes 5 mins before
  };
      ...
      <div className="sticky top-0 z-50 bg-header shadow-xl">
        <header className="w-full p-3 flex items-center justify-between text-white">
          <div className="flex items-center gap-4">
            <h1 className="text-lg font-black tracking-widest uppercase">KALYAN 777 Jackpot</h1>
          </div>
          <Button variant="outline" className="bg-background/10 border-white/20 text-white hover:bg-white/20 h-8 px-3 gap-1 pointer-events-none">
            <CustomWalletIcon className="h-5 w-5" />
            <span className="font-bold text-white">{totalBalance.toFixed(0)}</span>
          </Button>
        </header>

        <div className="px-4 pb-3 space-y-3">
          <div className="flex gap-2">
            <NavButton icon={History} label="Bid History" href="/jackpot/bid-history" />
            <NavButton icon={Trophy} label="Win Report" href="/jackpot/win-history" />
            <NavButton icon={BarChart2} label="Chart" href="/jackpot/charts" />
          </div>

          <div className="grid grid-cols-1">
            <RateCard title="Jodi Digit Rate" rate={`10 - ${appSettings.jackpotRateJodi || 1000}`} />
          </div>
        </div>
      </div>

      {/* Games List - Vertical Chronological Stack */}
      <main className="flex-1 p-4 space-y-3 mt-2 flex flex-col">
        {loading ? (
          <div className="flex justify-center py-20"><Loader className="h-10 w-10 text-primary" /></div>
        ) : jackpotGames.length === 0 ? (
          <p className="text-center text-gray-400 py-20">No jackpot games available.</p>
        ) : (
              ...
              <Card key={game.id} className="bg-white rounded-2xl border-none shadow-lg overflow-hidden animate-in fade-in slide-in-from-bottom-4 shrink-0">
                <CardContent className="p-3 flex items-center justify-between">
                  <div className="flex flex-col gap-1">
                    <div className="inline-flex">
                      <span className="bg-[#154c79] text-white text-[9px] font-bold px-2 py-0.5 rounded-full uppercase">
                        KALYAN 777 <span className="text-yellow-400">JACKPOT</span>
                      </span>
                    </div>
                    <div className="flex flex-col">
                      ...
                      <span className={cn(
                        ...
                        {running ? "Running now" : "Closed"}
                      </span>
                    </div>
                  </div>

                  <div className="flex flex-col items-center">
                    <div className="bg-gray-800 text-white px-4 py-1.5 rounded-full font-black tracking-[0.2em] text-sm shadow-inner">
                      {game.result || '**'}
                    </div>
                  </div>
                  ...
                  <Button 
                    ...
                    onClick={() => running && router.push(`/play-jackpot/${game.id}`)}
                  >
